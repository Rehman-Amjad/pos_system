        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              TextHelper().mNormalText(text: "Items",color: Colors.white,size: 18.0),
              const SizedBox(height: defaultDrawerHeadHeight-10.0),
              TextHelper().mNormalText(text: "Add new Items",color: Colors.white70,size: 14.0),
              const SizedBox(height: defaultDrawerHeadHeight + 20.0),
              ItemsRegistrationForm(
                edit: provider.parameters?['edit'] ?? 'false',
                code: provider.parameters?[Constant.KEY_ITEM_CODE] ?? "no code",
                name: provider.parameters?[Constant.KEY_ITEM_NAME] ?? 'Enter Item Name',
                category: provider.parameters?[Constant.KEY_ITEM_CATEGORY] ?? 'Enter category',
                uom: provider.parameters?[Constant.KEY_ITEM_UOM] ?? 'Enter uom',
                stock: provider.parameters?[Constant.KEY_ITEM_STOCK] ?? 'Enter stock',
                quantity: provider.parameters?[Constant.KEY_ITEM_QUANTITY] ?? 'Enter quantity',
                purchasePrice: provider.parameters?[Constant.KEY_ITEM_PURCHASE_PRICE] ?? 'Enter Purchase Price',
                salePrice: provider.parameters?[Constant.KEY_ITEM_SALE_PRICE] ?? 'Enter Sale Price',
                joinDate: provider.parameters?[Constant.KEY_VENDORMAN_JOIN_DATE] ?? 'select date',
